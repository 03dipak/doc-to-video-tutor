...
| 14 | A rejected plan sample is persisted beside the build, but the persisted artifact does not reproduce the failure that rejected it | **OPEN** — observed in `mod03_gates_v012_001`. The build log rejected sample 1 with `plan opening SlideTextNotEnglish`, yet the saved `*.sample1.plan.json` has no Devanagari in its opening and re-runs `guard_plan` with **zero** problems. So the plan written next to the rejection cannot be used to audit why the sample was discarded, which defeats the purpose of landing every sample. `_sanitize_opening` is *not* the cause: it strips only field-name leakage such as a trailing " opening" and leaves non-Latin script untouched, and `guard_plan` does not mutate its input — both verified directly. The stripping therefore happens inside `plan_lesson`, between the model returning and the artifact being written, and the mechanism is not yet identified. Until it is, a rejected sample's recorded problems cannot be trusted to be reproducible from its own artifact. |
| 15 | Deck text overflows its container: the "Why THIS (not the alternative)" card was a fixed 0.72 in with a 0.6 in text box, and a bullet row added a flat 0.12 in above 90 characters | **CLOSED** — a PowerPoint textbox does not clip, so undersized text draws over whatever sits below rather than disappearing; the fit check used the same fixed height and so reported room that did not exist. Rows and cards are now sized from measured wrapped text (`_est_text_height`), and the verdict-code chip label is inset inside its own card instead of overhanging it by 0.08 in - harmless for `0 PASS`, not for `4 CONFIG ERR`. Added `_audit_layout`, which reports text overflow, shape overlap beyond 0.12 in, and safe-area breaches after every deck build. Measured on `mod03_gates_v012_002`: 3 overflows up to 0.36 in and 4 chip overlaps at 2.18x0.34 in; the rebuilt deck reports clean. Only the mechanically decidable checks are automated - whether a decorative element earns its place stays a human judgement, because a gate that cannot be computed reliably only teaches people to ignore gates. |
| 17 | The deck paginated by bullet COUNT, so a page of four long bullets still overflowed and the row stack dropped the remainder | **CLOSED** — `_scene_pages` enforces the plan-level max of four bullets a page, which is the right instinct (overflow becomes more slides, not smaller text) but the wrong unit: bullet length varies by an order of magnitude. `_paginate_by_height` re-paginates the deck on measured height, so pagination and layout use one measurement instead of disagreeing. Measured: at ~676-character bullets, count-only pagination renders 1 of 4 and height pagination renders all 4. Below ~256 characters the two are identical, so the change is inert for ordinary lessons. `_est_wrapped_lines` also now counts a single unbreakable token as the number of lines it actually occupies; it previously reported a 900-character identifier as one line, which is an under-report and the only unsafe direction. |
| 22 | A block was drawn a full inch past the safe bottom, and no check noticed | **CLOSED** — slide 9 of `mod03_gates_v012_010` drew a payload card from 5.59 in to 7.78 in on a 7.5 in slide. The card was sized *correctly*, so no overlap fired and the layout audit reported nothing wrong: the defect was not crowding, it was off-canvas content. Cause: the diagram block advanced the local `y` cursor without going through `_RowStack`, so the stack's cursor fell behind and `stack.free` overstated the room left. The stack then admitted a block that could not fit. Two cursors, one of them stale, is exactly what a single shared budget exists to prevent. The diagram now reserves through the stack, and the code panel reads the cursor back instead of advancing a second one by the same amount. |
     The durable guard is a **safe-area invariant**, not another overlap check: no content shape may end below 6.9 in. That catches off-canvas content regardless of which bookkeeping caused it, and it runs against both the real plan and a synthetic worst case so the invariant does not depend on one lesson. A second test asserts no block in `build_pptx` advances the local cursor without the shared stack, which is the specific way this recurred. |
| 21 | Slide chrome overlapped on every content slide, and the layout audit reported clean | **CLOSED** — reported on slides 2,4,5,6,7,9,10 of `mod03_gates_v012_009`. The audit had said "no findings" because `_AUDIT_OVERLAP_TOL` was 0.12 in, set earlier to absorb exactly these collisions on the grounds that they "read as intentional". They were defects, and the tolerance was a detector silenced rather than a bug fixed — the wrong direction, and the reason a real defect survived several rounds of layout work. Two chrome causes: the eyebrow box was a round 0.4 in tall for 12 pt text and hung 0.08 in into the title (10.00 in wide), and the title box was 11.0 in from x=0.42, running to 11.42 and under the page counter at x=11.20 (0.22 in). Both are now measured. The payload and value cards still used a flat 0.3 in per line and spilled under their own card, and the `ANALOGY` label box overlapped its own body by 0.02 in on a hard-coded offset — both now measured too. Tolerance is 0.01 in. Verified by an independent geometry check written separately from the studio's own auditor: 0 non-contained overlaps across the 11-slide deck. |
| 20 | The pipeline reported a render-blocking condition as "proceeding anyway" and then refused the build | **CLOSED** — `repeating narration phrase` is a *soft* prefix at the plan stage, where it only decides whether to resample, and *hard* at the pre-render gate, where it refuses. One defect, two severities: on `mod03_gates_v012_008` the plan stage printed "WARNING: repeating narration phrase detected ... proceeding anyway", the build spent 43 s on the LLM and a full TTS pass, and the same plan was then refused with "RENDER-BLOCKING: 1 banned narration phrase(s) unresolved". This is the second instance of the §22.2 policy-drift pattern in one session, the first being the soft `unspoken visual claim` prefix; both come from the same stringly-typed soft/hard contract. The plan stage now consults the render gate before promising to continue, and says plainly that the plan and audit are written for repair while no media will be produced. **The refusal itself was correct** and is unchanged: the narration genuinely repeated the trigram `consistent verdicts units`, spanning the join between the design decision ("...critical for consistent verdicts.") and the next fragment ("Units define 'too much'"). A trigram across a fragment boundary is invisible to a per-fragment check, so the repair chain left it and the gate caught it — which is the gate working. |
| 19 | Takeaway cards advanced by a constant 1.2 in step while the card height was guessed from character count | **CLOSED** — observed on slide 11 of `mod03_gates_v012_007` as three overlaps 5.90 in wide, a full column. This is the same defect already fixed for scene bullets, in a path the layout audit had never been run against: earlier decks passed only because their takeaways happened to be short, which is precisely how a latent bug survives. Takeaways now flow through two columns by measured height, and overflow becomes a continuation slide rather than a shorter summary — dropping half of eight long takeaways to protect the layout is the wrong trade on the deck's most important slide. Measured: 8 of 8 placed across three pages. The page count is measured before rendering so the counter stays honest; a fixed `rows_per_col` guess would have printed `10 / 11` twice. |
| 18 | Deck content defects the layout audit cannot see: the literal JSON key `opening` printed on the title slide, and one diagram repeated across 3 of 9 scenes | **CLOSED** — the layout audit only knows geometry, so neither was visible to it. Field-name leakage is now stripped at *render* time as well as plan time: the deck is also built from saved plans, which can predate the sanitiser, and a renderer must not trust upstream hygiene for text it puts on a slide. `_dedupe_visual_diagrams` drops a diagram an earlier scene already showed, applying to the visual channel the rule Layer A already applies to repeated narration sentences — a repeat teaches nothing the first telling did not. The earliest use is kept so the teaching content and the first, fullest use of the visual survive. 3 duplicates removed on `mod03_gates_v012_002`. |
| 16 | On-screen structured facts the narration never speaks | **CLOSED (gate added)** — the exit-code badge row shows `0 PASS` through `4 CONFIG ERR` while the narration mentioned only 3 and 4, and the `active.json` payload slide showed `schema_version` / `baseline_id` / `path` without saying them. The viewer reads facts the lesson never explains, and the enrichment that puts those blocks on screen is what created the risk. `_unspoken_visual_claims` now reports uncovered badge codes/labels and payload keys as a soft finding. Two details matter: it compares in the *spoken* alphabet, because the badge says `3` while the narrator says "exit three" and a raw substring test reports a spoken fact as unspoken - the same asymmetry the reveal matcher had to solve; and it is soft, not blocking, because the remedy is a narration decision and silently enumerating a legend into the script is content injection the narration contract forbids. |
...
### 22.7 Three reviews of PPTX overlap — consensus and disposition

Three independent reviews were asked the same question: how should a generator
prevent shape overlap when content length is not known in advance? They diverge
on tooling and agree on the diagnosis.

**They corrected the framing, and the correction matters.** All three rejected
the responsive-HTML mental model. A `.pptx` slide is a fixed-aspect canvas that
scales uniformly, like an SVG with a `viewBox`, so a 13" laptop and a 32" monitor
cannot produce overlap — there is no reflow to trigger. The cause is content
volume against a fixed text-box budget decided at generation time. Designing for
viewport variation would have built the wrong thing; designing for *content*
variation is the whole problem.

| Proposal | Disposition | Reason |
|---|---|---|
| Measure content before assigning coordinates | **adopted** | `_est_text_height` drives every row and card; a pre-render audit reports overflow and overlap after each deck build (§20.2 defect 15) |
| One vertical budget per slide, resolved centrally | **adopted** | `_RowStack` replaced six independent `if y + need > max_h` checks. Overflow is resolved by priority — lowest, then bottom-most — instead of clipping |
| If text does not fit, paginate rather than shrink below the font floor | **adopted** | `_paginate_by_height` re-paginates the deck on measured height (§20.2 defect 17) |
| Lock the deck to one 16:9 canvas, one size per deck | **already true** | 13.333 x 7.5 in, set once in `build_pptx` |
| Enforce content budgets upstream at the LLM | **partly adopted** | Max four bullets per page is enforced; a per-field character budget is not, and is the cheapest remaining prevention |
| `MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE` as the fit mechanism | **rejected** | Two of the three reviews warned against it as primary: it silently produces different font sizes across slides and cannot be pre-audited. This pipeline rejects or repaginates deterministically rather than letting a renderer compensate quietly |
| HTML/CSS -> headless browser -> PPTX | **rejected** | Puts Chromium and a Node runtime in the render path, and yields non-editable output. The deck must stay native and editable; the video already rasterises deterministically with Pillow |
| Percentage-based coordinates from design tokens | **deferred** | Correct in principle — the code still holds literals like `Inches(0.52)`. A wide refactor with real regression risk, for a benefit that only appears if a second aspect ratio is ever shipped |
| Verify Pillow and PPTX text rendering agree | **open** | They are separate measurement systems today: `slides.py` wraps at 60 characters, `pptx.py` measures in inches. Claude's point is right that a deck can look correct as a PNG and differ in PowerPoint |
| Render decks through LibreOffice headless in CI | **open, blocked** | No LibreOffice on this machine, so it cannot be validated locally. XML geometry is not the same as rendered output, which makes the current audit necessary but not sufficient |
